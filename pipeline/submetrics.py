...
COMPONENTS = [
    ('volume', 'Volume', 'How much work he gets', [
        ...
        ('pass_att_pg', 'Pass attempts / game',
         lambda a, s, m: _d(a.get('pass_att'), a.get('gp')), True, 30, True, True),
    ]),
    ('efficiency', 'Efficiency', 'What he does with it', [
        ...
        ('pass_ypa', 'Yards per pass attempt',
         lambda a, s, m: a.get('pass_ypa') or 0, True, 20, True, True),
        ('cmp_pct', 'Completion %',
         lambda a, s, m: a.get('cmp_pct') or 0, True, 15, True, True),
        ('pass_rtg', 'Passer rating',
         lambda a, s, m: a.get('pass_rtg') or 0, True, 15, True, False),
        ('sack_rate', 'Sacks taken / dropback',
         lambda a, s, m: _d(a.get('pass_sack'),
                            (a.get('pass_att') or 0) + (a.get('pass_sack') or 0)),
         False, 10, True, False),
    ]),
    ('redzone', 'Red zone', 'Work where points are scored', [
        ...
        ('pass_rz_pg', 'Pass attempts in the red zone / game',
         lambda a, s, m: _d(a.get('pass_rz_att'), a.get('gp')), True, 25, True, True),
        ('pass_td_pg', 'Passing touchdowns / game',
         lambda a, s, m: _d(a.get('pass_td'), a.get('gp')), True, 25, True, True),
    ...
    ('explosive', 'Explosive', 'Big plays', [
        ...
        ('pass_deep', 'Passing touchdowns of 40+ yards',
         lambda a, s, m: a.get('pass_td_40p') or 0, True, 20, True, True),
    ...
    ('reliability', 'Reliability', 'Can you count on him', [
        ...
        ('int_pg', 'Interceptions thrown / game',
         lambda a, s, m: _d(a.get('pass_int'), a.get('gp')), False, 30, True, True),
        ('fum_pg', 'Fumbles / game',
...
ONLY = {
    # passing means nothing for anyone but a quarterback, and 'touches' means nothing
    # FOR one - a quarterback's attempts and targets are not the same kind of thing
    'pass_att_pg':   ['QB'],
    'pass_ypa':      ['QB'],
    'cmp_pct':       ['QB'],
    'pass_rtg':      ['QB'],
